...
import { Box, GridItem, Link, Text, Image } from '@chakra-ui/react'
...
const questions = [
  ...
  },
  {
    ...
    answer: 'No, once you have executed the migration, the $PRINTS tokens will be burned, and in exchange, you will receive your Voxelglyph.',
  },
  {
    question: "Fow how long will members be able to migrate their $PRINTS?",
    answer: 'The migration website will be launched on August 16th and will remain open indefinitely, so you can migrate your $PRINTS whenever you feel like.',
  },
  {
  ...
  },
  {
    ...
    answer: 'Yes, Voxelglyph will be an edition piece of up to 2,000 pieces, the only difference between them being their token IDs, which will be assigned in order of migration.',
  },
  {
...
const Info = () => {
  return (
    <Box pt={{ base: 14, md: '88px' }} pb={{ base: 10, md: 20 }} color={'white'} opacity={0.9}>
      <Grid>
        <GridItem colStart={{ xl: 2 }} colSpan={{ base: 4, sm: 6, md: 12, xl: 10 }}>
          ...
          <Box mb={28}>
            {questions.map((item, index) => {
              ...
              return (
                <Box py={8} borderBottomColor="white" borderBottomWidth={!isLastChild ? 1 : 0} key={index} id={`q-${index}`}>
